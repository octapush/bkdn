$(function(){
	var globalVars= {
		...
		servicePath: adam.format('%1/%2',adam.currentHost(),base_url.invoice+'invoice'),
		input:{
			post:[
				$('input#code'),
				$('input#code_customer'),
	...
	var main = {
			...
			buildTable: function(){
				globalVars.tableConfigs = globalVars.tableId.dataTable({
                    ...
                    [
                        ...
                        { title:'Kode Proyek',data:"no_kontrak" },
                        { title:'No Invoice',data:"no_invoice" },
                        { title:'No Faktur Pajak',data:"no_faktur" },
                        { title:'Tgl',data:"tgl" },
                            ...
                            if('delete'==bType){
                                $.messager.confirm('My Title', 'Apakah anda ingin menghapus data ini!?', function(r){
					                if (r){
					                    ...
					                    main.CRUDS.delete(data.data.id.toString(),data.index);
					                }
					            });
                            }else{
                            	var dt = data.data;
                            	...
                            	if(dt!=null || !dt!=undefined){
                            		console.log(dt);
                            		// globalVars.input.post[0].combobox('setValue',dt.code);
                              //   	globalVars.input.post[1].combobox('setValue',dt.no_kontrak);
                                	globalVars.input.post[2].textbox('setValue',dt.no_kontrak);
                                	globalVars.input.post[3].textbox('setValue',dt.no_invoice);
                                	globalVars.input.post[4].textbox('setValue',dt.no_faktur);
                                	globalVars.input.post[5].datebox('setValue',dt.tgl);
                                	globalVars.input.post[6].textbox('setValue',dt.penerima);
                            	}
		...
		ROUTINES: {
			...
			initInputData: function(){
            	...
            	globalVars.input.post[3].textbox('setValue','');
            	globalVars.input.post[4].textbox('setValue','');
            	globalVars.input.post[5].datebox('setValue','');
            	globalVars.input.post[6].textbox('setValue','');
			},
			data: function(){
				return {
					code 			:globalVars.input.post[0].val(),
					code_customer	:globalVars.input.post[1].val(),
					no_kontrak 		:globalVars.input.post[2].val(),
					no_invoice 		:globalVars.input.post[3].val(),
					no_faktur 		:globalVars.input.post[4].val(),
					tgl 			:globalVars.input.post[5].val(),
					penerima 		:globalVars.input.post[6].val(),
				}
			}
		},
		CRUDS: {
