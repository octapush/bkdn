$(function(){
	var globalVars= {
		...
		servicePath: adam.format('%1/%2',adam.currentHost(),base_url.employee+'employee'),
		input:{
			post:[
				$('input#code'),
				$('input#name'),
                          ...
                          var data = main.ROUTINES.getSelectedRow(that);
                            ...
                            if('delete'==bType){
                            	...
                            	if(dt!=null || !dt!=undefined){
                                	...
                                	globalVars.input.post[2].textbox('setValue',dt.placeofbirth);
                                	newDate = $.trim(dt.birthday)=='00/00/0000' || dt.birthday==null?'':dt.birthday;
                                	globalVars.input.post[3].datebox('setValue',newDate);
                                	globalVars.input.post[4].combobox('setValue',dt.gender);
                                	globalVars.input.post[5].textbox('setValue',dt.address);
                                	globalVars.input.post[6].textbox('setValue',dt.phone);
                                	globalVars.input.post[7].textbox('setValue',dt.username);
                                	globalVars.input.post[8].textbox('setValue',dt.password);
                            	}
                               main.ROUTINES.showMasterForm('EDIT',{title:'UPDATE DATA CUSTOMER',width:500,height:350});
                            }
                    });
			},
			btnShowFormMaster: function(){
				...
				oBtn.unbind().bind('click',function(){
					...
					var bType = that.attr('data-tag');
						...
						oBtnUpdate.hide();
					if("Add"==bType){
						main.ROUTINES.showMasterForm('ADD',{title:'NEW EMPLOYEE',width:500,height:350});
					}
				});
		...
		ROUTINES: {
			...
			data: function(){
				return {
					...
					placeofbirth 	:globalVars.input.post[2].val(),
					birthday 		:globalVars.input.post[3].val(),
					gender 			:globalVars.input.post[4].val(),
					address 		:globalVars.input.post[5].val(),
					phone 			:globalVars.input.post[6].val(),
					username 		:globalVars.input.post[7].val(),
					password 		:globalVars.input.post[8].val()
				}
			}
		},
		CRUDS: {
