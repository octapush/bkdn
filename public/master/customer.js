$(function(){
	var globalVars= {
		...
		servicePath: adam.format('%1/%2',adam.currentHost(),base_url.employee+'customer'),
		input: {
			post:[$('input#code'),$('input#name'),$('input#address'),$('input#npwp'),$('input#phone')]
		}
	};
		...
		UI: {
			...
			buildTable: function(){
				globalVars.tableConfigs = globalVars.tableId.dataTable({
                    ...
                    ajax: {
                        url: globalVars.servicePath+'/getcustomer',
                        type: 'POST',
                        data: { method:'GETALL'},
                        dataType: 'JSON',
                            error: function(e) {
                          ...
                          var data = main.ROUTINES.getSelectedRow(that);
                            if('delete'==bType){
                                ...
                                $.each(data,function(key,cVal){
                                	globalVars.input.post[0].val(cVal.code);
                                	globalVars.input.post[1].val(cVal.name);
                                	globalVars.input.post[2].val(cVal.address);
                                	globalVars.input.post[3].val(cVal.npwp);
                                	globalVars.input.post[4].val(cVal.phone);
                                });
                                main.ROUTINES.showMasterForm('EDIT',{title:'UPDATE DATA CUSTOMER',width:500,height:370});
			...
			btnShowFormMaster: function(){
				...
				oBtn.unbind().bind('click',function(){
					...
					var bType = that.attr('data-tag');
						...
						oBtnUpdate.hide();
					if("Add"==bType){
						main.ROUTINES.showMasterForm('ADD',{title:'NEW CUSTOMER',width:500,height:370});
					}
				});
		...
		ROUTINES: {
			...
			initInputData: function(){
				$.each(globalVars.input.post,function(key,data){
					var that = $(this);
					var cVal = that.attr('data-mandatory');
					if(cVal!='true')
						data.val('');
				})
			},
